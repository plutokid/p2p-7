  ...
  Outpost.views = {
    ...
    map: Backbone.View.extend({
      ...
      initialize: function() {
		...
		// if the device is mobile than hide the other menu and show the mobile menu
		if(isMobileDevice()){
	  ...
	  setMapCenter:function(place){
		/*if (place.geometry.viewport) {
			...
			this.$el.gmap3({
			  map: {
				options: {
					"center": place.geometry.viewport
				}
			  }
			});
		} else {
      ...
      render: function() {
		...
		if(isMobileDevice()){
			new Outpost.views.mobileTopBar();
		}else{
    ...
    // Mobile topbar view
    // =======================================================
    mobileTopBar: Backbone.View.extend({
      el: '.mobile-top-bar',
      template: _.template($('#tmpl-resultInfo').html()),

      events: {
        'click .filter-button': 'showHideFiltersDropDown',
        'click .listing-button': 'showHideListingDropDown',
        'click .submit-filter': 'filterReulsts',
        'click .filter-applyto': 'toggleFilterTo'
      },
      ...
      initialize: function() {
        ...
        this.initializeFilterGUI();
		this.initTypeahead();
		$(".listing-dropdown").append($('ul.nav-tabs'));
		$(".listing-dropdown").append($('.tab-content'));
		$(".tab-content").height($(window).height()-50);
      },
	  initTypeahead: function() {
        var origLocation = $(".searchbox")[0];
        var options = {
          types: ['(cities)']
        };
        var autocomplete = new google.maps.places.Autocomplete(origLocation, options);
		var dis = this;
		google.maps.event.addListener(autocomplete, 'place_changed', function() {
			var place = autocomplete.getPlace();

			dis.clearAndFetch();
			var orignalLocation = $(".searchbox").val();
			var origlatLng = Outpost.helpers.genRdmLL(orignalLocation);
			Outpost.values.origLocation = orignalLocation;
			Outpost.values.origLocationLat = origlatLng[0];
			Outpost.values.origLocationLng = origlatLng[1];
			Outpost.mvc.views.map = new Outpost.views.map();
			Outpost.mvc.views.map.setMapCenter(place);
		});
	  ...
	  showHideFiltersDropDown:function(){
		// Toggle the menu display.
		$('.filter-dropdown').toggle();
		$('.listing-dropdown').hide();

	  },
	  showHideListingDropDown:function(){
		// Toggle the menu display.
